        ...
        class DataFeed {
            ...
            trades(amount, price, count) {
                ...
                return Waves.AssetPair.get(amount, price)
                    .then((pair) => fetch(`${WavesApp.network.datafeed}/trades/${pair.toString()}/${count}`));
            }
            ...
            candles(amount, price, interval, count) {
                ...
                count = count || 100;
                return fetch(`${WavesApp.network.datafeed}/candles/${amount}/${price}/${interval}/${count}`)
                    .then((r) => r.json());
            }
            ...
            candlesFrame(amount, price, interval, from, to) {
                const url = WavesApp.network.datafeed;
                return fetch(`${url}/candles/${amount}/${price}/${parseInt(interval, 10)}/${from}/${to}`)
                    .then((r) => r.json());
            }
